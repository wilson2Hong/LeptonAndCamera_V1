...
void LeptonThread::run()
{
    ...
    while(true) {
        ...
        for(int j=0;j<PACKETS_PER_FRAME;j++) {
            ...
            read(spi_cs0_fd, result+sizeof(uint8_t)*PACKET_SIZE*j, sizeof(uint8_t)*PACKET_SIZE);             //**************摄像头修改的地方*******************//
            int packetNumber = result[j*PACKET_SIZE+1];                                                      //**************摄像头修改的地方*******************//
...
void LeptonThread::Identify(Mat& frame){
    ...
    Mat gray_image;//灰度图
    Mat binary_image;//二值图
    Mat dilate_img;//膨胀后的图片
    ...
    for (int index = 0; index < contours.size(); index = hierarchy[index][0])
    {
        ...
        sendStr.append(space);                                               //串口输出的坐标修改位置
